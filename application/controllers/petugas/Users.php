...
class Users extends CI_Controller {
	...
	public function index(){ 
		...
		$data['role']	= $this->db->query("SELECT * from tb_role_user where id != '1' order by id desc")->result();
		$this->template->load('template_new', 'petugas/users', $data);
	}
	// detail
	public function detail($id)
	{
		...
		$this->template->load('template_new', 'petugas/users_detail', $data);
	}
 	 ...
 	 public function add()
    {
       ...
       tampil_alert('success','Berhasil','Data User Berhasil di buat');
        redirect(base_url('petugas/Users'));
